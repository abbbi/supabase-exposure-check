...
SENSITIVE_FIELD_PATTERNS = [
    ...
    r"\bapikey\b",
    r"\bauth[_-]?key\b",
    r"\bapplication[_-]?key\b",
    r"\bsecret\b",
    r"\bprivate[_-]?key\b",
    r"\bsecret[_-]?key\b",
    r"\bshared[_-]?secret\b",
    r"\btoken\b",
    r"\bjwt\b",
    r"\baccess[_-]?token\b",
    ...
    r"\bauth\b",
    r"\bauth[_-]?code\b",
    r"\bauthorization[_-]?code\b",
    r"\bsession[_-]?id\b",
    r"\bsession[_-]?key\b",
    r"\bsession[_-]?secret\b",
    r"\brecovery[_-]?code\b",
    r"\bbackup[_-]?code\b",
    r"\bverification[_-]?code\b",
    r"\botp\b",
    r"\btwo[_-]?factor\b",
    r"\b2fa[_-]?secret\b",
    r"\b2fa[_-]?code\b",
    ...
    r"\bsocial[_-]?security[_-]?number\b",
    r"\bdriver[_-]?license\b",
    r"\bdrivers[_-]?license\b",
    r"\blicense[_-]?number\b",
    r"\bpassport[_-]?number\b",
    r"\bpassport[_-]?id\b",
    r"\bnational[_-]?id\b",
    r"\bnational[_-]?identifier\b",
    r"\btax[_-]?id\b",
    r"\buser[_-]?id\b",
    r"\baccount[_-]?id\b",
    r"\bcustomer[_-]?id\b",
    ...
    r"\bswift[_-]?code\b",
    r"\bbic\b",
    r"\bpayment[_-]?method\b",
    r"\bpayment[_-]?info\b",
    r"\bpayment[_-]?details\b",
    r"\bsalary\b",
    r"\bincome\b",
    r"\bwage\b",
    r"\bpayroll\b",
    ...
    r"\bhome[_-]?address\b",
    r"\bwork[_-]?address\b",
    r"\bbilling[_-]?address\b",
    r"\bip[_-]?address\b",
    r"\bipv4\b",
    r"\bipv6\b",
    r"\blocation\b",
    r"\bcoordinates\b",
    r"\bgps[_-]?coordinates\b",
    r"\blat\b",
    r"\blong\b",
    r"\blatitude\b",
    r"\blongitude\b",
    ...
    r"\bpatient[_-]?id\b",
    r"\binsurance[_-]?number\b",
    r"\bhealth[_-]?insurance\b",
    r"\bmedical[_-]?insurance\b",
    r"\bdiagnosis\b",
    r"\btreatment\b",
    # Device & Hardware Identifiers
    r"\bdevice[_-]?id\b",
    r"\bdevice[_-]?identifier\b",
    r"\bdevice[_-]?uuid\b",
    r"\bmac[_-]?address\b",
    r"\bmacaddr\b",
    r"\bimei\b",
    r"\bserial[_-]?number\b",
    r"\bhardware[_-]?id\b",
    ...
    r"\bssh[_-]?key\b",
    r"\blicense[_-]?key\b",
    r"\bsubscription[_-]?key\b",
    r"\bactivation[_-]?key\b",
    # Biometric Data
    r"\bfingerprint\b",
    r"\bbiometric\b",
    r"\bfacial[_-]?recognition\b",
    # Legal & Compliance
    r"\bcase[_-]?number\b",
    r"\blegal[_-]?document\b",
    # Internal/System
    r"\binternal[_-]?note\b",
    r"\badmin[_-]?note\b",
    r"\bconfidential\b",
]
...
NON_SENSITIVE_FIELDS = [
    ...
    r"\bimage\b",
    r"\bavatar\b",
    r"\bsearch[_-]?vector\b",
    r"\bvector\b",
    r"\bauthor[_-]?links\b",
    r"\bexample[_-]?emails\b",  # These are example/public data
]
...
def safe_get(url, headers=None, **kwargs):
    ...
    try:
        return requests.get(url, headers=final_headers, timeout=TIMEOUT, **kwargs)
    except requests.exceptions.SSLError:
        print(f"  [!] SSL error → retrying insecure: {url}")
        try:
            return requests.get(
                url, headers=final_headers, timeout=TIMEOUT, verify=False, **kwargs
            )
...
def scan_js(js_url):
    ...
    content = r.text
    return (JWT_REGEX.findall(content), extract_supabase_urls(content))
...
def analyze_table_for_sensitive_data(rows, max_samples=100):
    ...
    for field in all_fields:
        ...
        non_null_values = [
            row.get(field) for row in sample_rows if row.get(field) is not None
        ][:10]
        ...
        for value in non_null_values:
            ...
            if (
                ...
                if (
                    "phone" in field_lower or "mobile" in field_lower
                ) and "phone_pattern" not in detection_reasons:
    ...
    if sensitive_fields:
        ...
        if has_high_severity:
            ...
            "email" in str(reason) or "phone" in str(reason)
            for field_data in field_analysis.values()
            for reason in field_data["reasons"]
        ):
...
def scan_site(site_url):
    ...
    findings = {"site": site_url, "vulnerable": False, "supabase_urls": [], "jwts": []}
    ...
    supabase_headers = {"apikey": jwt, "Authorization": f"Bearer {jwt}"}
    ...
    try:
        ...
        for table in tables:
            ...
            if status == 200:
                ...
                if is_vulnerable:
                    print(
                        f"    🚨 {table}: {len(rows)} rows - VULNERABLE ({vuln_level}) - Sensitive fields: {', '.join(sensitive_fields)}"
                    )
                else:
                    print(
                        f"    [+] {table}: {len(rows)} rows - Public data (no sensitive fields detected)"
                    )
    ...
    critical_tables = [
        s for s in vulnerable_tables if s.get("vulnerability_level") == "critical"
    ]
    high_tables = [
        s for s in vulnerable_tables if s.get("vulnerability_level") == "high"
    ]
    medium_tables = [
        s for s in vulnerable_tables if s.get("vulnerability_level") == "medium"
    ]
    ...
    if vulnerable_tables:
        ...
        print(
            f"     - Total vulnerable: {len(vulnerable_tables)}/{len([s for s in summary if s.get('dumped')])} accessible tables"
        )
        ...
        if critical_tables:
            ...
            for t in critical_tables:
                print(
                    f"       • {t['table']} - Fields: {', '.join(t.get('sensitive_fields', []))}"
                )
...
def parse_args():
    ...
    parser.add_argument(
        "--url", help="Single website URL to scan (e.g., https://example.com)"
    )
    parser.add_argument(
        "--file", "-f", help="File containing list of URLs to scan (one per line)"
    )
    parser.add_argument(
        "--output",
        "-o",
...
def main():
    ...
    if args.url:
        ...
        print(
            "[-] No input provided. Use --url to scan a single site or --file to scan from a file."
        )
